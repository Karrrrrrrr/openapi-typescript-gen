            ...
            for (let methodName in methods) {
                ...
                if (parameters.length) {
                    ...
                    pathParams.forEach(item => {
                        paramsBody += `\t${this.transformObjectKey(item.name)}: ${this.getType(item.schema)},\n`
                        if (item.schema.$ref) {
                            addGroupType(tag, this.$refToType(item.schema.$ref))
                        }
                    })
                    queryParams.forEach(item => {
                        query += `\t\t${this.transformObjectKey(item.name)}${item.required ? '' : '?'}: ${this.getType(item.schema)},\n`
                        if (item.schema.$ref) {
                            addGroupType(tag, this.$refToType(item.schema.$ref))
                        }
                    })
                    headerParams.forEach(item => {
                        headers += `\t\t${this.transformObjectKey(item.name)}${item.required ? '' : '?'}: ${this.getType(item.schema)},\n`
                        if (item.schema.$ref) {
                            addGroupType(tag, this.$refToType(item.schema.$ref))
                        }
                    })
